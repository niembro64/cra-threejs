# Galaxy Destroyer

- Category: Video Games
- Type: vertical-scrolling shooter
- Date: 2024
- Public URL: https://games.niemo.io/space
- Declared stack: Phaser, React, TypeScript, FL Studio
- Platforms: desktop and mobile web

## What Eric built

Galaxy Destroyer is Eric’s browser-based take on the classic vertical space shooter. The player moves through waves of enemies, dodges projectiles, chooses weapons, and eventually confronts a boss. It supports simple mobile controls as well as desktop mouse and keyboard input, with sound and music production represented by FL Studio in the project stack.

## Implementation and interaction details

The live page opens on a “Galaxy Destroyer” start screen framed by large illustrated planets. Starting the game launches a Phaser canvas rather than a conventional form-driven web page. The portfolio and public preview show the expected scrolling-shooter loop: player movement, firing, enemy patterns, hazards, weapon variation, and a larger boss encounter.

The exact number of levels, weapons, enemy types, or boss phases is not documented in the public data, so the voice agent should answer those questions cautiously instead of extrapolating from the genre.

## What it demonstrates

The game combines a real-time update loop, sprite and projectile management, collision handling, enemy behavior, progression, responsive controls for two input modes, React page integration, TypeScript state, and original audio production. It is a concise example of Eric carrying a game from mechanics through presentation on both phone and desktop browsers.


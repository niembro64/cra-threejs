...
import type { Service, ServicesProps } from '../types/types';
import type { ProjectStatus } from '../data/myData';
...
const services: Service[] = [
  ...
  },
  {
    ...
    description: ['Download YouTube videos as MP4 or extract audio as MP3.', 'Simple paste-and-download interface.'],
    projectStatus: 'disabled',
...
const Services: React.FC<ServicesProps> = ({ isThin }) => {
  return (
    <section className={`px-4 py-12 ${isThin ? 'bg-black/80' : ''}`}>
      ...
      <div className={`grid items-stretch gap-y-16 ${isThin ? 'grid-cols-1' : 'grid-cols-2 gap-x-8'}`}>
        {services.map((service) => (
          <article
            key={service.title}
            className="flex h-full w-full min-w-0 flex-col rounded-2xl transition-all duration-300"
          >
            <div className="mb-4 flex flex-row items-center justify-center text-center">
              <h3
                className={`pixel-font uppercase leading-tight text-white ${isThin ? 'text-4xl' : 'text-[clamp(1.75rem,2.4vw,3rem)]'}`}
              >
            ...
            <div className="flex flex-1 flex-col px-4 py-0">
              {service.description.map((desc) => (
                <div key={desc} className="mb-0 text-center text-xl text-blue-100">
                  {desc}
                </div>
              ))}

              <div className="mt-auto pt-4 text-center">
                {service.projectStatus === 'ok' && ((isThin && service.mobile) || (!isThin && service.desktop)) ? (
                  <FancyButton
                  ...
                  <button
                    ...
                    disabled
                  >
                    {service.projectStatus !== 'ok' ? 'Offline' : isThin ? 'Desktop Only' : 'Mobile Only'}
                  </button>
                )}
              </div>
            </div>
